...
const docToLog = (id: string, data: Record<string, unknown>, userId: string): WorkoutLog => ({
  id,
  user_id:       userId,
...
/*
  ...
  the same guarded day/session writer as the online write below, so a replay
...
export const useWorkoutLogs = (planId?: string) => {
  ...
  const query_ = useQuery({
    ...
    queryFn: async () => {
      ...
      return snap.docs.map(d => docToLog(d.id, d.data() as Record<string, unknown>, user.uid));
    },
    enabled: !!user && !!planId,
    staleTime: 1000 * 60 * 5,
  });
  ...
  const toggleDayMutation = useSupabaseAction({
    ...
    onMutate: async ({ workoutDateStr, completed }: ToggleDayParams) => {
      ...
      queryClient.setQueryData(queryKey, (old: WorkoutLog[] = []) => {
        const idx = old.findIndex(l => l.workout_day === workoutDateStr && isDaySessionLog(l));
        if (idx > -1) {
    ...
    onError: (_err: unknown, _v: ToggleDayParams, context: { previousLogs?: WorkoutLog[] } | undefined) => {
